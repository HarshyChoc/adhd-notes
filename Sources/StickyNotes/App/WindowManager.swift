...
class WindowManager: ObservableObject {
    ...
    func openWindow(for note: Note, coordinator: AppCoordinator) {
        ...
        if windowControllers[note.id] != nil {
            return
        }
    ...
    func closeWindow(for noteId: UUID) {
        ...
        windowController.closeFromModelSync()
    ...
    func hideWindow(_ noteId: UUID) {
        windowControllers[noteId]?.window?.orderOut(nil)
    }

    func showWindow(_ noteId: UUID) {
        guard let controller = windowControllers[noteId] else { return }
        controller.moveToActiveSpaceIfNeeded()
